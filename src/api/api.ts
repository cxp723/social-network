...
const apiInstance = axios.create({
  ...
  headers: {
    "API-KEY": "fc0db4b0-b81e-4d15-99bc-4c6431ebd432",
  },
});
...
export const getPhotosForAlbum = (numberOfPhotos: number, page: number) => {
  return photoAPI
    .get("curated?per_page=" + numberOfPhotos + "&page=" + page)
...
export const profileAPI = {
  ...
  uploadPhoto(photoFile: File) {
    ...
    return apiInstance
      .put<ResponseType<ResultCodes, { photos: PhotoType }>>(
        "profile/photo",
        formData,
        {
          headers: {
...
export const followAPI = {
  follow(userId: number) {
    const follow = apiInstance
      .post<ResponseType>("follow/" + userId)
      .then((response) => response.data);
    return follow;
  },
  unfollow(userId: number) {
    const unfollow = apiInstance
...
type LoginResponse = ResponseType<
  ResultCodes | ResultCodeWithCaptcha,
  { userId: number }
>;
