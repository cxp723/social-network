...
const actions = {
  ...
  setUserInfo: (name: string | null, photo: string | null) =>
    ({ type: "SET_USER_INFO", payload: { name, photo } } as const),
  setCaptchaPicture: (captcha: string | null) =>
    ({ type: "SET_CAPTCHA_PICTURE", payload: { captcha } } as const),
...
export const login = (
  ...
  return async (dispatch) => {
    ...
    if (response.resultCode === ResultCodes.Success) {
      ...
      response.resultCode === ResultCodeWithCaptcha.CaptchaRequired &&
        dispatch(getCaptcha());
      dispatch(actions.toggleIsFetching(false));
    }
  };
