...
export const usersActions = {
  followSuccess: (userId: number) =>
    ({ type: "FOLLOW_SUCCESS", userId } as const),
  unfollowSuccess: (userId: number) =>
    ({ type: "UNFOLLOW_SUCCESS", userId } as const),
  ...
  setUsersTotalCount: (usersTotalCount: number) =>
    ({ type: "SET_USERS_TOTAL_COUNT", usersTotalCount } as const),
  togglePreloader: (isFetching: boolean) =>
    ({ type: "TOGGLE_IS_FETCHING", isFetching } as const),
  toggleFollowingButton: (userId: number) =>
    ({ type: "TOGGLE_FOLLOWING_BUTTON", userId } as const),
};
...
const usersReducer = (
  ...
  switch (action.type) {
    ...
    case "TOGGLE_FOLLOWING_BUTTON":
      return {
        ...state,
        followingUsers: state.followingUsers.some(
...
//Thunk creators
type ThunkCreatorType = ThunkAction<
  Promise<void>,
  AppStateType,
  unknown,
  UsersActionsType
...
export const getUsers = (
  ...
  return async (dispatch) => {
    ...
    if (!term) {
      ...
      data = await usersAPI.getUsersFromServer(
        currentPageFromEnd,
        pageSize,
        null,
        friend
...
const followFlow = async (
  dispatch: Dispatch<UsersActionsType>,
  userId: number,
  apiMethod: (userId: number) => Promise<ResponseType>,
  actionMethod: (userId: number) => UsersActionsType
) => {
