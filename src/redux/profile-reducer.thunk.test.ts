import { getProfile, profileActions, savePhoto, updateProfile } from './profile-reducer';
...
test('getProfile thunk works correctly', async () => {
    profileAPIMock.getProfile.mockReturnValue(Promise.resolve(successProfileResponse))
    const getProfileThunk = getProfile(1)
    await getProfileThunk(dispatchMock)
    expect(dispatchMock).toBeCalledWith(profileActions.setProfile(successProfileResponse))
})

const successUploadPhotoResponse: ResponseType<ResultCodes, { photos: PhotoType }> = {
...
test('save photo thunk works correctly', async () => {
    profileAPIMock.uploadPhoto.mockReturnValue(Promise.resolve(successUploadPhotoResponse))
    const savePhotoThunk = savePhoto(require('../images/ava.jpg'))
    await savePhotoThunk(dispatchMock)
    expect(dispatchMock).toBeCalledTimes(3)
    expect(dispatchMock).toHaveBeenNthCalledWith(1, profileActions.toggleUploading())
    expect(dispatchMock).toHaveBeenNthCalledWith(2, profileActions.setPhoto(successUploadPhotoResponse.data.photos))
    expect(dispatchMock).toHaveBeenNthCalledWith(3, profileActions.toggleUploading())
})
...
test('update profile thunk works correctly', async () => {
    profileAPIMock.setProfile.mockReturnValue(Promise.resolve(successProfileUpdateResolve))
    const profileUpdateThunk = updateProfile(profileDataForUpdate)
    await profileUpdateThunk(dispatchMock, getStateMock, {})
    expect(dispatchMock).toBeCalledTimes(4)
    expect(dispatchMock).toHaveBeenNthCalledWith(1, profileActions.toggleUpdating(true))
    //expect(dispatchMock).toHaveBeenNthCalledWith(2, getProfile(1))
    expect(dispatchMock).toHaveBeenNthCalledWith(3, profileActions.updatingProfileInProcess(false))
    expect(dispatchMock).toHaveBeenNthCalledWith(4, profileActions.toggleUpdating(false))
})
