  ...
  ({
    ...
    for (let p = leftLimit; p <= rightLimit; p++) {
      pages.push(
        <div
          ...
          className={`${classes.button} ${p === page && classes.selectedPage}`}
        >
          {p}
        </div>
      );
    ...
    return (
      <div className={classes.paginator}>
        {currentSection > 1 && (
          <button className={classes.button} onClick={goPrev}>
            ←
          </button>
        )}
        <div className={classes.pages}>{pagesCount > 1 && pages}</div>
        {currentSection < paginatorSectionsCount && (
          <button className={classes.button} onClick={goNext}>
            →
          </button>
        )}
      </div>
